"""T1b: same GRU, 1000 updates; original phase3_t01.py preserved."""
...
def prepare():
    ...
    streams = {}
    for split, seed, count in [('train',93014,2000), ('dev',93015,64)]:
    ...
    path = ROOT/'configs/phase3_t01b_manifest.json'
    if path.exists():
...
def gru_case(manifest, delay):
    ...
    result=dict(delay=delay,updates=1000,parameters=sum(p.numel() for p in model.parameters()),model_load_s=load_s,
                train_s=sum(times),first_step_s=times[0],mean_step_s=sum(times)/1000,valid_training_targets=2000*(32-delay),new_symbol_positions=64000,
...
def worker(a):
    ...
    hashes={str(p):sha(ROOT/p) for p in ['phase3_t01b.py','PROTOCOLLO_T1B_T4_T5.md','configs/phase3_t01b_manifest.json','dataset/prepared_v1/manifest.json','bench_runtime.py']}
    if a.case=='t0':
...
def main():
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_t01b')
    try:result=worker(a)
