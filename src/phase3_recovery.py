...
def execute(a):
    ...
    model=build_cns();initialize(model,a.seed)
    if a.case=='memory':
        ...
        dev=text_pairs(reserved,128,'validation');epochs=2
    ...
    try:
        ...
        save_model(checkpoint,model,dict(protocol='PROTOCOLLO_FASE_3_C.md',case=a.case,delay=a.delay,updates=updates,lr=a.lr))
    ...
    return dict(ok=True,result=result,config=vars(a),peak_allocated_mb=peak,final_allocator_mb=alloc,
                source_sha256={f:hashlib.sha256(Path(f).read_bytes()).hexdigest() for f in ['phase3_recovery.py','phase3_extended.py','PROTOCOLLO_FASE_3_C.md','fly_lm.py','fly_core.py','fly_interfaces.py','bench_runtime.py']})
...
def main():
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_recovery')
    try:r=execute(a)
