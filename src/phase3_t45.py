...
def load_rows():return json.loads((ROOT/'configs/phase3_t01_manifest.json').read_text())['streams']
...
def t5(a):
    ...
    checkpoint=Path(a.output).with_suffix('.diagnostic.pt');save_model(checkpoint,model,dict(protocol='PROTOCOLLO_T1B_T4_T5.md',length=a.length,delay=a.delay,updates=200))
    return dict(length=a.length,delay=a.delay,updates=200,valid_targets=total,stream_segments=entries,train=tr,dev=dv,train_s=sum(times),mean_step_s=sum(times)/200,first_step_s=times[0],equivalence=equivalence,sensitivity=grad,
                initial_checkpoint_sha256=sha(ROOT/'results/phase3_t2_final.pt'),checkpoint_sha256=sha(checkpoint),data_sha256=sha(ROOT/'configs/phase3_t01_manifest.json'))
...
def worker(a):
    ...
    hashes={f:sha(ROOT/f) for f in ['phase3_t45.py','PROTOCOLLO_T1B_T4_T5.md','fly_lm.py','fly_core.py','fly_attention.py','fly_interfaces.py','bench_runtime.py']}
...
def main():
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_t45')
    try:r=worker(a)
