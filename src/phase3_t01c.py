"""T1c: same GRU, 2000 updates; original phase3_t01.py preserved."""
...
def prepare():
    ...
    streams = {}
    for split, seed, count in [('train',93014,4000), ('dev',93015,64)]:
    ...
    path = ROOT/'configs/phase3_t01c_manifest.json'
    if path.exists():
...
def gru_case(manifest, delay):
    ...
    result=dict(delay=delay,updates=2000,parameters=sum(p.numel() for p in model.parameters()),model_load_s=load_s,
                train_s=sum(times),first_step_s=times[0],mean_step_s=sum(times)/2000,valid_training_targets=4000*(32-delay),new_symbol_positions=128000,
...
def worker(a):
    ...
    manifest=prepare()
    old=json.loads((ROOT/'configs/phase3_t01_manifest.json').read_text())
    assert manifest['streams']['train'][:400]==old['streams']['train'] and manifest['streams']['dev']==old['streams']['dev']
    assert manifest['audit_ids']==old['audit_ids']
    hashes={str(p):sha(ROOT/p) for p in ['phase3_t01c.py','PROTOCOLLO_T1B_T4_T5.md','configs/phase3_t01c_manifest.json','dataset/prepared_v1/manifest.json','bench_runtime.py']}
    if a.case=='t0':
...
def main():
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_t01c')
    try:result=worker(a)
