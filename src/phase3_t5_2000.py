...
def t5(a):
    ...
    train=blocks(rows['train'][:1334],a.delay);dev=blocks(rows['dev'],a.delay)
    ...
    for index,pair in enumerate(train):
        ...
        for segment,(x,y) in enumerate(pair):
            ...
            if len(times) in (200,500,1000,2000):
    ...
    checkpoint=Path(a.output).with_suffix('.diagnostic.pt');save_model(checkpoint,model,dict(protocol='PROTOCOLLO_ESTENSIONE_FASE_3.md',length=a.length,delay=a.delay,updates=2000))
    return dict(curve=curve,length=a.length,delay=a.delay,updates=2000,valid_targets=total,stream_segments=entries,train=tr,dev=dv,train_s=sum(times),mean_step_s=sum(times)/2000,first_step_s=times[0],equivalence=equivalence,sensitivity=grad,
...
def worker(a):
    ...
    hashes={f:sha(ROOT/f) for f in ['phase3_t5_2000.py','PROTOCOLLO_ESTENSIONE_FASE_3.md','fly_lm.py','fly_core.py','fly_attention.py','fly_interfaces.py','bench_runtime.py']}
...
def main():
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_t5_2000')
    try:r=worker(a)
