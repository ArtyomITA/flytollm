...
def variant(model,name):
    if name=='threshold5':
        ...
        larger=build_cns(5)
        larger.interfaces.load_state_dict(model.interfaces.state_dict())
        larger.attention.load_state_dict(model.attention.state_dict())
        return larger
...
def run(a):
    ...
    return dict(specification=model.specification(),variant=a.variant,initial_dev=initial,dev=final,train=tr,curve=curve,updates=200,targets=targets,train_s=sum(times),
...
def main():
    p=argparse.ArgumentParser();p.add_argument('--variant',choices=['base','p1','p2','d1','d2','h1','threshold5'],required=True);p.add_argument('--output',required=True)
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_threshold5_trial')
    try:
        import torch
