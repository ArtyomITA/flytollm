...
def execute(a):
    ...
    return dict(ok=True,result=result,equivalence=equivalence,specification=specification,artificial_parameter_sha256=artificial_sha,config=vars(a),peak_allocated_mb=peak,final_allocator_mb=alloc,
                source_sha256={f:hashlib.sha256(Path(f).read_bytes()).hexdigest() for f in ['phase3_compare.py','phase3_capture16.py','phase3_extended.py','PROTOCOLLO_FASE_3_D.md','fly_lm.py','fly_core.py','fly_interfaces.py','bench_runtime.py']})
...
def main():
    ...
    a=p.parse_args()
    if not a.worker:return supervise(a,worker_module='phase3_compare')
    try:r=execute(a)
