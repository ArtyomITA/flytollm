...
| C15 | Sinapsi rimesse all'init | CE del checkpoint K8 con i pesi sinaptici riportati all'init; copertura per superclasse. | Quanto lavoro fa l'apprendimento sinaptico. | 5 min | FALLITO per un bug dello script (corretto): da rifare nella coda 8c. | UnboundLocalError: `record +=` in closure → record.add_(). rerun: phase8_c15_revert_weights.py --kind relthr su K8. |
