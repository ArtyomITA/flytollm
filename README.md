...
- **18 settembre, C15**: nel modello migliore (3,541 a 8000 update) rimettere TUTTI i pesi sinaptici ai valori iniziali cambia la CE di 0,0006 nat. Il nucleo è un reservoir fisso con interfacce allenate. Con lr sinaptico 10-100 volte più grande i pesi si muovono fino al 15% ma la CE non cambia (C14); con il gradiente "allargato" ai presinaptici vicini alla soglia gli archi che ricevono gradiente passano dal 3% al 15-23% (M0). Perché, con fonti: [`reports/RICERCA_CORE_NON_IMPARA.md`](reports/RICERCA_CORE_NON_IMPARA.md) (regime lazy, gradient starvation, inizializzazione quiescente, softplus che taglia il passo di 34 volte, rapporto segnale/rumore). In coda: allenare solo il nucleo con interfacce congelate, inizializzazione a fluttuazione, interfacce lente, scala dell'uscita, canali d'ingresso per modalità sensoriale, porte sui neuroni di convergenza multisensoriale (6 raggiunti da tutte le modalità entro 2 sinapsi, 10.444 entro 3).
- **18 settembre notte, perché il nucleo non impara (pannello E0 sul modello migliore)**: l'81% dei neuroni è muto sul DEV e sta in mediana a 9 deviazioni sotto la soglia (solo il 7% nella finestra utile 1-3); il 61% non entra mai nella finestra del gradiente surrogato. Il passo effettivo della softplus è ×0,029 (taglio ×34). Il gradiente che arriva alle sinapsi dei neuroni attivi è rumore: rapporto segnale/rumore mediano 0,22 contro 0,25 del rumore puro su 16 campioni, solo 4 sinapsi su 1.000 hanno una direzione stabile. Con gradiente allargato + passo ×10 le sinapsi si muovono 18 volte di più (un quinto degli archi) e la CE non cambia di un millesimo (4,185 contro 4,187): regime lazy. In coda: init a fluttuazione (E1), interfacce lente (E3), nucleo da solo (E4), scala dell'uscita (E2).
- **18 settembre, mattina: il cablaggio vale 1,68 nat come reservoir, allenarlo vale zero**. Nucleo identità (sinapsi spente e congelate, neuroni isolati): 5,864; sinapsi reali congelate ai valori iniziali: 4,182; sinapsi allenate: 4,187 (tutto a 8+8, 2000 update, Muon). Gradiente allargato + passo sinaptico ×10/×100: i pesi si muovono fino al 28% e la CE non cambia (4,183 / 4,185 / 4,200).
- **Attention dentro il giro del connettoma (logica looped-transformer) è neutra**: due letture per token 4,196, una lettura a ogni sottopasso 4,178, con cache della prima lettura 4,186, senza identificativo di passo 4,198, contro 4,187 con una lettura sola (costi da +6% a ×1,8). Token iniettato solo al primo sottopasso: 4,914 (+0,73): la corrente d'ingresso sostenuta è il supporto vitale della dinamica (spike ÷100 senza).
- Porte a conduttanza 4,188 (nulla); guadagno della corrente esterna per superclasse 4,198 (nulla, ma il modello alza la corrente ×2 sulle grandi popolazioni intrinseche e la inverte sui sensori ascendenti: chiede più eccitazione, +35% di spike, senza guadagno di CE).
- **18 settembre pomeriggio, correzione importante: le sinapsi imparano, ma serve tempo e serve che il testo attraversi il cervello**. Stesso test (sinapsi rimesse ai valori iniziali) sui checkpoint del vecchio standard a porte anatomiche: contributo delle sinapsi allenate 0,008 nat a 2000 update, 0,089 a 8000, **0,243 a 26.247**. Con porte random (standard attuale) a 8000: 0,0006, perché il testo raggiunge la lettura in una sinapsi e le interfacce scavalcano il cervello. Il rapporto segnale/rumore del gradiente sinaptico è uguale al rumore anche a 26k (0,235): il segnale si accumula col numero di update. In coda: porte anatomiche nel default attuale a 8000 update, con e senza arousal.
- **Fattoriale grafo × attention (8+8, Muon, 2000 update)**: reale + attention 4,184 · reale senza 4,282 · rimescolato + attention 4,243 · rimescolato senza 4,321. L'attention vale 0,10 nat ed è additiva (interazione +0,02). **Il grafo reale è avanti sul rimescolato a gradi conservati in entrambe le coppie (+0,06 / +0,04)**: prima volta nella suite; un solo seme di rimescolamento, sotto la soglia 0,10: indizio da confermare.
- Omeostasi di risveglio per tipo cellulare: con bersaglio 0,02 e scarto di soglia fino a 0,9 l'allenamento diverge (gradiente 1e10 a 8+8, 1e16 a 12+12); impulsi e onde di arousal sono stabili, raddoppiano le sinapsi mosse ma a 2000 update quelle sinapsi non fanno lavoro utile.
- Trasferimento di profondità (N6c): un modello allenato a 8+8 valutato a 12+12 perde 0,04, a 4+4 perde 0,26; da 4+4 a 12+12 il salto è 0,34.
